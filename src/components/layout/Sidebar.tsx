...
export function Sidebar({ open, onToggle, variant }: SidebarProps) {
    ...
    return (
      <div className="fixed inset-0 z-40 bg-[rgba(20,22,18,0.45)]" data-testid="sidebar-backdrop" onClick={onToggle}>
        <div
          ...
          className="flex h-full w-1/2 flex-col gap-0.5 bg-[#fbfbfa] px-4 pt-[54px] pb-6 shadow-[2px_0_16px_rgba(0,0,0,0.18)]"
        ...
        >
          <button
            ...
            aria-label={toggleLabel}
            onClick={onToggle}
            className="-ml-2.5 mb-2.5 flex min-h-[44px] min-w-[44px] items-center justify-center self-start border-none bg-transparent text-2xl text-[#6b6b62]"
          >
            ☰
          </button>
          <div className="mt-[5px] mb-4 -ml-[5px] flex items-center gap-2 px-2.5 font-bold text-[#1a1a1a]">
            <span className="h-[18px] w-[18px] flex-shrink-0 rounded-[5px] bg-[#2f6f52]" />
            <span className="text-lg">하운 쉐어하우스</span>
          </div>
          <div className="flex items-center gap-2 rounded-md bg-[#eaf2ee] px-2.5 py-2 text-xs font-bold text-[#2f6f52]">
            <span className="w-[18px] flex-shrink-0 text-center">📊</span>
            관리비 정산
          </div>
          <a
            href="#"
            className="flex items-center gap-2 rounded-md px-2.5 py-2 text-xs font-medium text-[#6b6b62] no-underline"
          >
            <span className="w-[18px] flex-shrink-0 text-center">🏠</span>
            입주자 관리
          </a>
  ...
  const content = (
    <div className="flex h-full w-[230px] flex-shrink-0 flex-col gap-0.5 bg-[#fbfbfa] px-3.5 pt-3 pb-6">
      ...
      <div className="ml-[5px] flex items-center gap-2 rounded-md bg-[#eaf2ee] px-2.5 py-2 font-bold text-[#2f6f52]">
        <span className="w-[18px] flex-shrink-0 text-center">📊</span>
        관리비 정산
      </div>
      <a
        href="#"
        className="ml-[5px] flex items-center gap-2 rounded-md px-2.5 py-2 font-medium text-[#6b6b62] no-underline"
      >
        <span className="w-[18px] flex-shrink-0 text-center">🏠</span>
        입주자 관리
      </a>
  ...
  return (
    <div className="fixed inset-0 z-40 bg-transparent" data-testid="sidebar-backdrop" onClick={onToggle}>
      <div
        data-testid="sidebar-drawer"
        className="absolute top-0 left-0 h-full w-[230px] shadow-[2px_0_16px_rgba(0,0,0,0.18)]"
        onClick={(e) => e.stopPropagation()}
      >
        {content}
      </div>
